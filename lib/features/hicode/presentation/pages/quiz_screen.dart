...
void _showZoomableImage(BuildContext context, String imageUrl) {
  ...
  final zoomableUrl =
      ImageOptimizer.getOptimizedUrl(imageUrl, width: 1200, quality: 90);
...
class QuizScreen extends StatelessWidget {
      ...
      child: BlocListener<QuizBloc, QuizState>(
        listener: (listenerContext, state) {
          if (state.status == QuizStatus.submitted) {
            _showResultDialog(context, state, quizId, chapterTitle,
                userName // <-- 3. PASS USERNAME KE DIALOG
                );
            ...
            WidgetsBinding.instance.addPostFrameCallback((_) {
              if (context.mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                      content: Text('Gagal submit: ${state.error}'),
            ...
            final examViewState =
                context.findAncestorStateOfType<__FinalExamViewState>();
            ...
            child: BlocBuilder<QuizBloc, QuizState>(
              builder: (context, state) {
                ...
                if (state.questions.isEmpty &&
                  ...
                  // Tambahkan pengecekan status success
                  return Center(
                      child: Text(
                          'Soal untuk ${state.quizId.startsWith('FINAL_') ? 'latihan final' : state.quizId == '00000000-0000-0000-0000-000000000000' ? 'ujian akhir' : 'kuis'} ini belum tersedia.'));
                }
                ...
                if (state.status == QuizStatus.success ||
                  ...
                  if (state.quizId == '00000000-0000-0000-0000-000000000000') {
                    ...
                    return _QuizView(
                        state: state,
                        quizId: state.quizId,
                        chapterTitle: chapterTitle); // Gunakan state.quizId
                  }
                }

                if (state.status == QuizStatus.failure) {
                  return Center(
                      child: Text(state.error ?? 'Gagal memuat kuis.'));
                }
  ...
  void _showResultDialog(BuildContext dialogContext, QuizState state,
      String currentQuizId, String? currentChapterTitle, String userName,) {
    ...
    if (quizId.startsWith('FINAL_')) {
      ...
      if (isPassed) {
        ...
        subtitle =
            'Hebat! Kamu lulus latihan final untuk "$materialNameForDisplay".';
        buttons = [
          _buildDialogButton(
            // Tombol ini dibuat sebagai fungsi helper statis atau di luar
        ...
        subtitle =
            'Kamu menjawab benar $correctAnswers dari $totalQuestions soal ($materialNameForDisplay). Minimal 60% benar diperlukan...';
        buttons = [
          _buildDialogButton(
            context: dialogContext, // Kirim context dialog
      ...
      subtitle =
          'Kamu menjawab benar $correctAnswers dari $totalQuestions soal. Skor akhirmu adalah ${result.score} poin...';
      if (!isPassed) {
        subtitle +=
            ...
            Navigator.of(dialogContext).pushReplacement(
              ...
              MaterialPageRoute(
                builder: (_) => ScoreScreen(
                  ...
                  userName: userName, // <-- 3. PASS USERNAME
                ),
              ),
            );
          },
      ...
      if (isPassed) {
        ...
        subtitle =
            'Kamu menjawab benar $correctAnswers dari $totalQuestions soal...';
        buttons = [
          _buildDialogButton(
            context: dialogContext, // Kirim context dialog
            text: 'Pelajari Ulang',
    ...
    showDialog(
      ...
      builder: (_) {
        // Tidak perlu dialogContext lagi di sini
        return Dialog(
          shape:
...
class _QuizView extends StatelessWidget {
  ...
  const _QuizView(
      {required this.state, required this.quizId, this.chapterTitle});
                      ...
                      child: Center(
                        ...
                        child: GestureDetector(
                          onTap: () => _showZoomableImage(
                              context, currentQuestion.imageUrl!),
                          child: Image.network(
  ...
  Widget _buildTopBar(
      BuildContext context, String quizId, String? chapterTitle) {
        ...
        children: [
          IconButton(
            onPressed: () async {
              ...
              final bool? shouldExit = await _showExitQuizDialog(
                  context, quizId); // <-- Gunakan parameter
            ...
            icon: Image.asset(
              ...
              height: 32,
              color: AppColors
                  .himfoBlue, // Beri warna biru agar terlihat di background cerah
            ),
          ),
          Expanded(
            child: Text(
  ...
  Future<bool?> _showExitQuizDialog(BuildContext context, String quizId) {
    ...
    String subtitle =
        'Anda sedang mengerjakan kuis. Keluar sekarang akan mengulang dari awal. Yakin ingin keluar?';
    ...
    if (quizId.startsWith('FINAL_')) {
      ...
      // <-- Perbaiki ID Ujian Akhir
      // Kondisi untuk Ujian Akhir keseluruhan
      subtitle =
          'Anda sedang mengerjakan Ujian Akhir. Keluar sekarang akan mengulang seluruh soal-soal. Yakin ingin keluar?';
      primaryButtonText = 'Keluar Ujian';
    } else {
      // Kondisi untuk kuis bab biasa
      title = 'Keluar dari Kuis';
      subtitle =
          'Kamu sedang mengerjakan kuis chapter ini. Jika keluar sekarang, akan diulang dari awal. Yakin ingin keluar?';
    ...
    return Padding(
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          child: Row(
            crossAxisAlignment:
                CrossAxisAlignment.start, // Align top jika ada gambar
            children: [
              // --- Lingkaran Pilihan ---
              Container(
                margin: const EdgeInsets.only(
                    top: 4), // Sedikit ke bawah agar sejajar teks
                width: 24,
                height: 24,
                decoration: BoxDecoration(
                      ...
                      Padding(
                        ...
                        child: ClipRRect(
                          ...
                          child: GestureDetector(
                            ...
                            child: Image.network(
                              // Gunakan URL optimasi yang sudah kita buat
                              ImageOptimizer.getOptimizedUrl(imageUrl,
...
class _BottomNavBar extends StatelessWidget {
  ...
  const _BottomNavBar(
      {required this.state, this.isFinalExam = false}); // Beri nilai default
...
class __FinalExamViewState extends State<_FinalExamView>
  ...
  void _startTimer() {
    _timer = Timer.periodic(const Duration(seconds: 1), (_) {
      if (_timeRemaining.inSeconds > 0) {
        ...
        if (context.mounted && !_isSubmitted) {
          // <-- TAMBAHKAN CEK !_isSubmitted
  ...
  Widget build(BuildContext context) {
    ...
    final currentQuestion =
        widget.state.questions[widget.state.currentQuestionIndex];
                ...
                child: SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 24),
                      ...currentQuestion.options.map((option) {
                        final isSelected = widget.state.selectedAnswers[
                                widget.state.currentQuestionIndex] ==
                            option.id;
                        return _OptionTile(
                          // Menggunakan kembali widget _OptionTile
                          optionKey: '',
                          optionText: option.optionText,
                          imageUrl:
                              option.imageUrl, // _OptionTile sudah di-update
                          isSelected: isSelected,
                          onTap: () {
                            context.read<QuizBloc>().add(AnswerSelected(
                                  questionIndex:
  ...
  Widget _buildFinalExamProgressBar(QuizState state) {
    final double progress = state.questions.isEmpty
        ? 0
